...
describe("CORS origin enforcement (integration)", () => {
  it("non-loopback host without env var throws actionable error", () => {
    expect(() => resolveCorsOrigin({ host: "0.0.0.0", corsEnv: undefined })).toThrow(/HOLYSHIP_CORS_ORIGIN must be set/);
  });
  ...
  it("whitespace-only HOLYSHIP_CORS_ORIGIN treated as unset", () => {
    expect(() => resolveCorsOrigin({ host: "0.0.0.0", corsEnv: "   " })).toThrow(/HOLYSHIP_CORS_ORIGIN must be set/);
  });
});
