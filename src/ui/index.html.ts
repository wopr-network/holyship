...
<title>Holyship Dashboard</title>
<style>
...
function doLogin() {
  ...
  TOKEN = v;
  sessionStorage.setItem('holyship-token', v);
  verifyToken();
}
  ...
  fetch('/api/ui/events/recent?limit=1', { headers: { Authorization: 'Bearer ' + TOKEN } })
    .then(r => {
      if (r.ok) {
        ...
        TOKEN = '';
        sessionStorage.removeItem('holyship-token');
      }
    })
...
const savedToken = sessionStorage.getItem('holyship-token') || sessionStorage.getItem('defcon-token');
if (savedToken) {
  TOKEN = savedToken;
