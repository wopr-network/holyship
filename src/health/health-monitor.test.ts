...
describe("HealthMonitor", () => {
  ...
  it("does nothing when all slots are healthy", async () => {
    ...
    const { client, reportFn } = makeHolyshipClient();
  ...
  it("catches holyship.report errors without crashing", async () => {
    const pool = new Pool(4);
    const { client, reportFn } = makeHolyshipClient();
  ...
  it("does not release slot if heartbeat arrives during holyship.report await", async () => {
    const pool = new Pool(4);
    // holyship.report takes time; during it, the heartbeat fires and updates lastHeartbeat
    ...
    monitor.start();
    // Advance enough to trigger check(); holyship.report is now awaiting
    await vi.advanceTimersByTimeAsync(FAST_CONFIG.heartbeatIntervalMs + 10);

    // Simulate heartbeat arriving while holyship.report is in flight
    slot.lastHeartbeat = Date.now();

    // Now resolve holyship.report
    resolveReport();
  ...
  it("stop() clears the interval", () => {
    const pool = new Pool(4);
    const { client } = makeHolyshipClient();
    const monitor = new HealthMonitor(pool, client, FAST_CONFIG);
